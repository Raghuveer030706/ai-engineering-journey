...
# Day 17 — FastAPI Wrapper

Wraps the full Day 16 multi-agent pipeline behind a single HTTP server.

## Endpoints

| Endpoint | Method | Description |
|---|---|---|
| `/ask` | POST | Run full pipeline, return answer + metadata |
| `/health` | GET | MCP server status, tool count, memory entries |
...
```powershell
conda activate ai-journey
cd phase5-production/day17-fastapi
...
# Ask a question
$body = '{"question": "What was the Phase 2 RAGAS score?"}'
...
```json
{
  "answer": "The Phase 2 RAGAS score was 0.827...",
  "sub_tasks": 1,
  "agents_used": ["rag"],
  "llm_calls": 5,
  "duration_ms": 36000,
...
| File | Purpose |
|---|---|
| `main.py` | FastAPI app, lifespan startup, two endpoints |
| `pipeline.py` | Async wrapper around Day 16 orchestration |
| `models.py` | Pydantic request/response schemas |

## Key fixes during build

- Hyphenated folder names break Python imports — use `sys.path.insert` not dotted imports
- `MCPClient.__init__()` takes no args — bare `MCPClient()` only
- `MCPClient` has no `close()` — connections are self-managed per call
- `Synthesizer` is a function not a class — `from synthesizer import synthesize`
- `LongTermMemory` uses module-level `DB_PATH` — no constructor args

## Known issue at end of Day 17

Response time ~60s per request.
Root cause: MCP subprocess spawned and torn down on every tool call.
Fixed in Day 18.

# Day 18 — Production Hardening

Reduces response time from 36s to 5.7s and adds per-stage request logging.

## What was added

| Feature | Detail |
|---|---|
| Per-stage logging | `planner`, `orchestrator`, `synthesizer` each timed separately |
| Request ID | Unique ID per request, written to `requests.log` |
| Timeout guard | `asyncio.wait_for` kills hung sub-tasks at 45s |
| Direct tool calls | Local tools bypass MCP subprocess entirely |
| Startup init | MCP client initialized once, reused across all requests |

## Performance progression

| Stage | Day 17 | After direct tools | After startup init |
|---|---|---|---|
| MCP init per request | 0ms | 5,000ms | 0ms |
| MCP subprocess per tool call | ~15s each | 0ms | 0ms |
| Planner | ~5s | ~1s | ~1s |
| Orchestrator | ~45s | ~3s | ~3s |
| Synthesizer | ~5s | ~1.5s | ~1.5s |
| **Total** | **~60s** | **~36s** | **~5.7s** |
...
# Check stage timings
Get-Content .\requests.log | ConvertFrom-Json | Select-Object -Last 1 | ConvertTo-Json -Depth 5
```
...
| File | Purpose |
|---|---|
| `main.py` | FastAPI app with startup MCP init |
| `pipeline.py` | Retry, timeout, stage logging, hybrid client |
| `direct_tools.py` | Local tool functions extracted from MCP server |
| `logger.py` | Per-request stage timing, written to `requests.log` |
| `cost.py` | LLM call counter and cost estimator |
| `models.py` | Pydantic schemas including `request_id` |

## Key design decisions

**Load at startup vs init per request**

| Component | Strategy | Reason |
|---|---|---|
| `SentenceTransformer` | Startup | 2-3s to load, stateless |
| `chromadb.PersistentClient` | Startup | File handle, stateless reads |
| `HybridClient` | Startup | 31s MCP init, stateless routing |
| `RequestLogger` | Per request | Tracks one request's timing |
| `CostTracker` | Per request | Tracks one request's token spend |
| Agent instances | Per request | Carry mid-run conversation history |

**HybridClient**
Drop-in replacement for `MCPClient`.
Local tools → direct Python function call (no subprocess).
Fetch tool → MCP subprocess (genuinely external, no alternative).
Agents required zero changes — same `call_tool()` signature.

## Known gaps
- `estimated_cost_usd` shows 0.0 — `CostTracker` counts calls but
  real token counts not yet wired from `response.usage`.
  To be fixed in Day 20 cleanup.
